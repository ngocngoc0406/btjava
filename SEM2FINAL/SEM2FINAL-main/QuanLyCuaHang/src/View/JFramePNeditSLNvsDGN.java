...
public class JFramePNeditSLNvsDGN extends javax.swing.JFrame {

    private SanPhamService sanphamService;
    /**
     * Creates new form JFramePNeditSLNvsDGN
     */
    public JFramePNeditSLNvsDGN() {
        initComponents();
    ...
    private void jbtNextActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtNextActionPerformed
        JFramePhieuNhapEdit.SLNhap = txtSLNhap.getText();
        JFramePhieuNhapEdit.DGNhap = txtDGNhap.getText();
        sanphamService = new SanPhamService();

        int row = JFramePhieuNhapEdit.tableCTPN.getSelectedRow();
        String masp = JFramePhieuNhapEdit.tableCTPN.getValueAt(row, 0).toString();
        ...
        } else if (Integer.parseInt(JFramePhieuNhapEdit.SLNhap) <= 0) {
            JOptionPane.showMessageDialog(this, "Vui lòng nhập lại số lượng nhập!");
            txtSLNhap.requestFocusInWindow();
        } else if (Integer.parseInt(JFramePhieuNhapEdit.DGNhap) <= 0) {
            JOptionPane.showMessageDialog(this, "Vui lòng nhập lại đơn giá nhập!");
            txtDGNhap.requestFocusInWindow();
        } else {

            JFramePhieuNhapEdit.tableCTPN.setValueAt(Integer.parseInt(JFramePhieuNhapEdit.SLNhap), row, 4);
            JFramePhieuNhapEdit.tableCTPN.setValueAt(Integer.parseInt(JFramePhieuNhapEdit.DGNhap), row, 5);
            Float ThanhTien = Float.valueOf(JFramePhieuNhapEdit.SLNhap) * Float.parseFloat(JFramePhieuNhapEdit.DGNhap);
            JFramePhieuNhapEdit.tableCTPN.setValueAt(String.format("%.0f", ThanhTien), row, 6);

            //KetToan
            CodeBanHang cbh = new CodeBanHang();
            JFramePhieuNhapEdit.txtTongTienMoi.setText(cbh.TinhTongTien(JFramePhieuNhapEdit.tableCTPN));
            Float TongTienMoi = Float.valueOf(JFramePhieuNhapEdit.txtTongTienMoi.getText().toString());
            Float TongTienCu = Float.valueOf(JFramePhieuNhapEdit.txtTongTienCu.getText().toString());
            float ThanhToan = TongTienCu - TongTienMoi;
            JFramePhieuNhapEdit.txtThanhToan.setText(String.format("%.0f", ThanhToan));
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        int row = JFramePhieuNhapEdit.tableCTPN.getSelectedRow();
        txtSLNhap.setText(JFramePhieuNhapEdit.tableCTPN.getValueAt(row, 4).toString());
        txtDGNhap.setText(JFramePhieuNhapEdit.tableCTPN.getValueAt(row, 5).toString());
